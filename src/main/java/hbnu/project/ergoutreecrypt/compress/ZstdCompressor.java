 ...
 *
 * <p>Zstandard 支持 1–22 档位（数值越大压缩率越高、速度越慢），本类将档位
 * 限制在 {@link #MIN_LEVEL}–{@link #MAX_LEVEL} 区间，越界值自动收敛到边界。
 * 解压侧通过帧头自动识别原始大小，无需在调用方保存/传递档位或长度信息。
 *
 * <p>提供字节数组与流式两套 API：字节数组用于内存内小载荷（隐写 payload），
 * 流式用于大文件（通用文件加密与隐写的文件级路径），内存占用恒定。
 *
 * @author ErgouTree
 */
