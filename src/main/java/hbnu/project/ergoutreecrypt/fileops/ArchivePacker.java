...
/**
 * 归档打包工具：加密后打包为 ZIP / GZ / TAR.GZ。
 * 若有密码，先对文件做 AES-256-CTR 加密，再写入归档。
 * <p>
 * 加密文件头：MAGIC(12) + salt(16) + IV(16) + ciphertext
 *
 * @author ErgouTree
 */
public final class ArchivePacker {
    ...
    /**
     * 将多个文件打包为单一归档（压缩永远是最后一步时使用）。
     *
     * <p>entries 中每个条目使用相对于 {@code baseDir} 的相对路径作为归档内条目名，
     * 以保留分卷碎片或文件夹的目录结构。若设置了密码，则对每个条目内容做 AES-256-CTR 加密。
     *
    ...
    public static void packEntries(Path output, Path baseDir, List<Path> entries,
        ...
        try {
            switch (effective) {
                ...
                case GZ -> packGz(output, entries.get(0), password);
    ...
    // ---- 通用：若设密码则先 AES 加密输入文件，返回临时加密文件路径 ----
    ...
    // 压缩包类型枚举
    public enum Format {ZIP, GZ, TAR_GZ}
}
